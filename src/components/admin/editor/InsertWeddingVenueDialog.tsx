...
import { fetchWeddingVenues, type WeddingVenue } from "@/lib/wedding-venues";
...
export function InsertWeddingVenueDialog({ open, onOpenChange, onSelect }: Props) {
  const [venues, setVenues] = useState<WeddingVenue[] | null>(null);
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Insert wedding venue
          </DialogTitle>
          <DialogDescription>
            Pick a wedding venue to embed inline in your article.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            {(["full", "compact"] as const).map((vv) => (
              <button
                key={vv}
                type="button"
                onClick={() => setVariant(vv)}
          ...
          <div className="max-h-[420px] overflow-y-auto rounded-xl border border-border">
            {loading && (
              <div className="flex items-center justify-center gap-2 p-8 text-sm text-muted-foreground">
                <Loader2 className="h-4 w-4 animate-spin" /> Loading wedding venues…
              </div>
            )}
            {!loading && filtered.length === 0 && (
              <div className="p-8 text-center text-sm text-muted-foreground">
                No wedding venues match your search.
              </div>
            )}
            {!loading &&
              filtered.map((v) => (
